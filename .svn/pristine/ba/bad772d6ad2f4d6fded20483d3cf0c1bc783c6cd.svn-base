...
#import "TCModelTool.h"
#import "TCTableTool.h"

@implementation TCSqliteModelTool
...
+ (BOOL)createTableWithModelClass: (Class)cls withUID: (NSString *)uid {
    ...
    // 1. 判断 表是否存在
    NSString *tableName = [TCModelTool getTableNameWithModelClass:cls];
    BOOL isTableExists = [TCTableTool isTableExists:tableName uid:uid];
    ...
    NSDictionary *ivarNameTypeDic = [TCModelTool getModelIvarNameSqlTypeDic:cls];
    ...
    if (!isTableExists) {
        ...
        return [TCSqliteTool dealSql:createTabelSQL withUID:uid];
    }
    
    
    // 表需要更新
    NSString *tmpTableName = [TCModelTool getTempTableNameWithModelClass:cls];
    ...
    NSArray *columnNames = [TCTableTool getTableAllColumnNames:tableName uid:uid];
    NSArray *modelColumnNames = [TCModelTool getModelIvarNames:cls];
    ...
    return [TCSqliteTool dealSqls:updateSqls withUID:uid];
...
+ (NSArray *)queryAllModels:(Class)cls uid: (NSString *)uid {
    ...
    NSArray *rowDicArray = [TCSqliteTool querySql:sql withUID:uid];
...
+ (NSArray *)queryModels:(Class)cls sql: (NSString *)sql uid: (NSString *)uid {
    ...
    NSArray *array = [TCSqliteTool querySql:sql withUID:uid];
...
+ (BOOL)deleteModel: (id)model uid: (NSString *)uid {
    NSString *tableName = [TCModelTool getTableNameWithModelClass:[model class]];
    ...
    return [TCSqliteTool dealSql:sql withUID:uid];
...
+ (BOOL)saveModel: (id)model uid: (NSString *)uid {
    ...
    NSArray *res = [TCSqliteTool querySql:sql withUID:uid];
    
    // 字段
    NSArray *columnNames = [TCModelTool getModelIvarNames:[model class]];
    ...
    if (res.count > 0) {
        ...
        return [TCSqliteTool dealSql:insertSql withUID:uid];
    }
...
+ (NSArray *)parseRowDicArray: (NSArray *)rowDicArray withModelClass: (Class)cls {
    ...
    // 模型真正的字段 - > 类型
    NSDictionary *modelColumnNames = [TCModelTool getModelIvarNameIvarTypeDic:cls];
...
+ (BOOL)isRequireUpdate:(Class)cls uid: (NSString *)uid {
    ...
    NSArray *tableColumnNames = [TCTableTool getTableAllColumnNames:[TCModelTool getTableNameWithModelClass:cls] uid:uid];
...
+ (NSDictionary *)naoTypeSQLRelation {
    ...
    return @{
             @(TCSqliteModelToolNAONot) : @"not",
             @(TCSqliteModelToolNAOAnd) : @"and",
             @(TCSqliteModelToolNAOOr) : @"or"
             };
