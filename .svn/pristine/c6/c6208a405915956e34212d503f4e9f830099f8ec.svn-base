...
//        _scanRect = CGRectFromString([self scanRectWithScale:1][0]);
//        self.output.rectOfInterest = _scanRect;
//        _tipTitle.text = @"将二维码/条码放入框中，即可自动扫码";
//
//        _tipTitle.center = CGPointMake(self.view.center.x, self.view.center.y + CGSizeFromString([self scanRectWithScale:1][1]).height/2 + 32);
//
...
- (void)drawTitle
{
    if (!_tipTitle)
    {
        ...
        _tipTitle.bounds = CGRectMake(0, 0, 330, 50);
        _tipTitle.center = CGPointMake(CGRectGetWidth(self.view.frame)/2, self.view.center.y + self.view.frame.size.width/2 - 20);
        _tipTitle.font = [UIFont systemFontOfSize:14];
