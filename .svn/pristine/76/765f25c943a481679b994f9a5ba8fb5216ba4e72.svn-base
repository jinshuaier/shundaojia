//
//  TCBindingDetialViewController.m
//  ShunDaoJia
//
...
#import "TCBindingDetialViewController.h"
#import "TCBindingCardVerViewController.h"
#import "TCProtocolViewController.h"

@interface TCBindingDetialViewController ()<UITextFieldDelegate>
@property (nonatomic, strong) UIButton *checkBtn;

@end

@implementation TCBindingDetialViewController
...
-(void)creatUI{
    ...
    for (int j = 0; j < titles2.count; j++) {
        ...
        if (j == 0) {
            ...
            UIButton *quesionBtn = [[UIButton alloc]initWithFrame:CGRectMake(WIDTH - 12 - 16, 19, 16, 16)];
            [quesionBtn setBackgroundImage:[UIImage imageNamed:@"黄问号"] forState:(UIControlStateNormal)];
...
-(void)clickSure:(UIButton *)sender{
    NSLog(@"点击了验证");
    TCBindingCardVerViewController *CardVer = [[TCBindingCardVerViewController alloc]init];
    [self.navigationController pushViewController:CardVer animated:YES];
}
