...
@property (nonatomic, assign) BOOL isSetMima; //是否设置IMA
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    delegate.isSDJSYTBack = YES;
    [[NSNotificationCenter defaultCenter] postNotificationName:@"SDJBackSS" object:nil];
...
- (void)viewDidLoad {
    ...
    //接受返回的通知，用来控制返回按钮
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(isSDJSYTBack) name:@"SDJSYTBack" object:nil];
    //初始化数据库
...
- (void)viewWillDisappear:(BOOL)animated{
    ...
    //记录返回按钮
    AppDelegate *delegate = (AppDelegate*)[[UIApplication sharedApplication]delegate];
    delegate.isSDJSYTBack = NO;
    
    if (self.isSetMima == NO){
