...
#define PWD_COUNT 6
#define DOT_WIDTH 10
@interface TCNoemptyPassController ()<UITextFieldDelegate,UIGestureRecognizerDelegate>
{
...
@implementation TCNoemptyPassController
...
-(void)request{
    [ProgressHUD showHUDToView:self.view];
        ...
        NSDictionary *dic = @{@"mid":midStr,@"token":tokenStr,@"timestamp":timeStr,@"payCode":self.passward};
        NSLog(@"%@",dic);
        NSString *signStr = [TCServerSecret signStr:dic];
        
        NSDictionary *paramters = @{@"mid":midStr,@"token":tokenStr,@"timestamp":timeStr,@"sign":signStr,@"payCode":self.passward};
        [TCNetworking postWithTcUrlString:[TCServerSecret loginAndRegisterSecretOffline:@"101012"] paramter:paramters success:^(NSString *jsonStr, NSDictionary *jsonDic) {
            ...
            if ([codeStr isEqualToString:@"1"]) {
                TCinputBankCController *inputVC = [[TCinputBankCController alloc]init];
