...
@interface TCWithaddDController ()<UITextFieldDelegate>
@property (nonatomic, strong) UIButton *checkBtn;
@property (nonatomic, strong) NSUserDefaults *userdefaults;

@end

@implementation TCWithaddDController
...
-(void)creatUI{
    ...
    UIView *view1 = [[UIView alloc]initWithFrame:CGRectMake(0, StatusBarAndNavigationBarHeight, WIDTH, 108)];
    ...
    for (int i = 0; i < titles1.count; i++) {
        ...
        if (i == 0) {
            ...
            _bankNametf.text = [NSString stringWithFormat:@"中国%@",self.cardname];
            ...
            _cardNumtf.text = [NSString stringWithFormat:@"%@",self.cardnum];
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    [man_textfield resignFirstResponder];
...
-(void)clickSure:(UIButton *)sender{
    ...
    UITextField *phone_textfield = (UITextField *)[self.view viewWithTag:1002];
        ...
        if (![BSUtils IsIdentityCard:card_textfield.text]){
            [TCProgressHUD showMessage:@"请输入正确的身份证号"];
        }else if (![BSUtils checkTelNumber:phone_textfield.text]){
...
-(void)request{
    ...
    NSString *cardno = self.cardnum;
    NSString *bankCode = self.cardid;
    NSString *bankName = self.cardname;
    NSString *name = man_textfield.text;
    NSString *idno = card_textfield.text;
    NSString *mobile = phone_textfield.text;
    NSDictionary *dic = @{@"mid":midStr,@"token":tokenStr,@"timestamp":timeStr,@"cardno":cardno,@"bankCode":bankCode,@"bankName":bankName,@"name":name,@"idno":idno,@"mobile":mobile};
    NSLog(@"dic:%@",dic);
    NSString *signStr = [TCServerSecret signStr:dic];
    
    NSDictionary *paramters = @{@"mid":midStr,@"token":tokenStr,@"timestamp":timeStr,@"sign":signStr,@"cardno":cardno,@"bankCode":bankCode,@"bankName":bankName,@"name":name,@"idno":idno,@"mobile":mobile};
    [TCNetworking postWithTcUrlString:[TCServerSecret loginAndRegisterSecretOffline:@"103010"] paramter:paramters success:^(NSString *jsonStr, NSDictionary *jsonDic) {
        ...
        if ([codeStr isEqualToString:@"1"]) {
            if ([self.entranceTypeStr isEqualToString:@"1"]) {
                for (UIViewController *controller in self.navigationController.viewControllers) {
                    if ([controller isKindOfClass:[TCMyMessageViewController class]]) {
                        TCMyMessageViewController *revise =(TCMyMessageViewController *)controller;
                ...
                for (UIViewController *controller in self.navigationController.viewControllers) {
                    if ([controller isKindOfClass:[TCMyBankCardViewController class]]) {
                        TCMyBankCardViewController *revise =(TCMyBankCardViewController *)controller;
                        [self.navigationController popToViewController:revise animated:YES];
                    }
                }
            }
