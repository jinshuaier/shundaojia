...
#import "TCReseNewViewController.h"

@interface TCResetViewController ()<UITextFieldDelegate>
...
@implementation TCResetViewController
...
-(void)creatUI{
    ...
    UILabel *textLabel = [[UILabel alloc]initWithFrame:CGRectMake(20, 32 + StatusBarAndNavigationBarHeight, WIDTH - 40, 22)];
    NSString *phoneStr = [self.defaults valueForKey:@"mobileStr"];
    textLabel.text = [NSString stringWithFormat:@"请输入手机%@收到的的短信验证码",phoneStr];
...
-(void)clickyan:(UIButton *)sender{
    NSLog(@"验证信息");
    TCReseNewViewController *restNew = [[TCReseNewViewController alloc]init];
    [self.navigationController pushViewController:restNew animated:YES];
